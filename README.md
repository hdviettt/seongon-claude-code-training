...
├── README.md               — file này
├── CLAUDE.md               — hướng dẫn cho Claude Code khi học viên hỏi
├── INDEX.md                — bản đồ topic → folder
├── LESSON_NOTES.md         — note giảng viên cho buổi học hiện tại
│
├── .claude/                — agents + skills cho Claude Code
│   ├── agents/
│   │   ├── giang-vien.md   — Giảng viên ảo (giải thích khái niệm)
│   │   └── tro-ly.md       — Trợ lý ảo (build sản phẩm)
│   └── skills/
│       └── audit-webapp/   — SKILL audit web app theo LESSON_NOTES
│
├── knowledge/              — bể kiến thức, topic-based
│   ├── 01-setup-claude-code/
│   │   ├── cai-dat/        — Windows, Mac, troubleshooting
│   │   └── giao-dien/      — phím tắt, commands, plan mode
│   ├── 02-tech-stack-web-app/
│   │   ├── lap-trinh-co-ban/
│   │   ├── frontend/
│   │   ├── backend/
│   │   ├── database/       — overview + hosted-providers.md
│   │   ├── security/       — overview + auth.md
│   │   ├── deployment/     — overview + platforms.md
│   │   ├── source-control/
│   │   └── recipes/        — static-to-fullstack, blog-admin-cms
│   ├── 03-mo-rong-claude-code/
│   │   ├── commands/
│   │   ├── skills/         — overview + cau-truc + examples/
│   │   └── ket-noi-ngoai/  — API / MCP / CLI
